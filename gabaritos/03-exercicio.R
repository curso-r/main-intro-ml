...
reg <- finalize_model(reg, select_best(tune_reg, metric = "rmse"))
reg_final <- fit(reg, b ~ x + y + r + g, data = df_1)  

df_1$b_reg <- predict(reg_final, df_1)$.pred
plot_img_df(df_1, b_e)
plot_img_df(df_1, b)
...
arv <- finalize_model(arv, select_best(tune, metric = "rmse"))
arv_final <- fit(arv, b ~ x + y + r + g, data = df_1)  

df_1$b_arv <- predict(arv_final, df_1)$.pred
plot_img_df(df_1, b_arv)
plot_img_df(df_1, b)
...
rmse(df_1, truth = b, estimate = b_reg)
rmse(df_1, truth = b, estimate = b_arv)
