...
especificacao_modelo1 <- decision_tree(
  cost_complexity = 0.01, # varia de 0 a 0.1
  min_n = 5, # varia de 2 a N
  tree_depth = 5, # varia de 1 a 30
  mode = "regression"
) %>% set_engine("rpart")

especificacao_modelo2 <- decision_tree(
  cost_complexity = 0.005, # varia de 0 a 0.1
  min_n = 7, # varia de 2 a N
  tree_depth = 8, # varia de 1 a 30
  mode = "regression"
) %>% set_engine("rpart")

especificacao_modelo3 <- decision_tree(
  cost_complexity = 0.001, # varia de 0 a 0.1
  min_n = 2, # varia de 2 a N
  tree_depth = 12, # varia de 1 a 30
  mode = "regression"
) %>% set_engine("rpart")
...
diamondsinho_com_previsao_g1 <- diamondsinho_com_previsao_longo %>%
  ggplot() +
  geom_point(aes(x, price), size = 3) +
  geom_step(aes(x, price_pred, color = modelo), size = 1) +
  theme_bw() +
  facet_wrap(~modelo)
diamondsinho_com_previsao_g1

# Observado vs Esperado
diamondsinho_com_previsao_g2 <- diamondsinho_com_previsao_longo %>%
  ggplot() +
...
diamondsinho_com_previsao_g3 <- diamondsinho_com_previsao_longo %>%
  ggplot() +
...
# Pontos observados + curva da f
diamondsinho_novos_com_previsao_g1 <- diamondsinho_novos_com_previsao_longo %>%
  ggplot() +
  geom_point(aes(x, price), size = 3) +
  geom_step(aes(x, price_pred, color = modelo), size = 1) +
  theme_bw() +
...
# Observado vs Esperado
diamondsinho_novos_com_previsao_g2 <- diamondsinho_novos_com_previsao_longo %>%
  ggplot() +
  geom_point(aes(price_pred, price, colour = modelo), size = 3) +
  geom_abline(slope = 1, intercept = 0, colour = "purple", size = 1) +
  theme_bw()
...
# resíduos vs Esperado
diamondsinho_novos_com_previsao_g3 <- diamondsinho_novos_com_previsao_longo %>%
  ggplot() +
  geom_point(aes(price_pred, price - price_pred, colour = modelo), size = 3) +
  geom_abline(slope = 0, intercept = 0, colour = "purple", size = 1) +
  ylim(c(-10000,10000)) +
